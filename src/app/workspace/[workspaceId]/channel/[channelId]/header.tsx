...
export const Header = ({ title }: HeaderProps) => {
  ...
  const [ConfirmDialog, confirm] = useConfirm(
    ...
    "This action can't be undone and all messages will be lost."
  );
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateChannel(
      { id: channelId, name: value },
      {
        ...
        onError: (error) => {
          toast.error("Couldn't update channel name");
        },
      }
    );
  };
    ...
    <div className="bg-white border-b h-[49px] flex items-center px-4 overflow-hidden">
      ...
      <Dialog>
        <DialogTrigger asChild>
          <Button
            variant="ghost"
            className="text-lg font-semibold px-2 overflow-hiddenw-auto"
            size="sm"
          >
              ...
              <DialogTrigger asChild>
                <div className="px-5 py-4 bg-white rounded-lg border cursor-pointer hover:bg-gray-50">
                  <div className="flex items-center justify-between">
                    ...
                    {member?.role === 'admin' && (
                      <p className="text-md text-[#1264a3] font-semibold hover:underline">
                        Edit
                      </p>
                    )}
                  </div>
